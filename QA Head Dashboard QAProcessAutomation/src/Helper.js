//***********Helper Functions for Add Reviewer and Remove Reviwer***************
...
//In Reviewer DB sheet Add new reviwer sheet link
function setReviewerSheet_CreateNewSheet(emailId, department) {
  ...
  if (rowIndex !== -1) {   // If the reviwer is present but does not have a sheet;
    ...
    reviewerSheetId = createCopyOfSpreadsheet(REVIEW_TEMPLATE_SHEET_ID, sheetName, department, "Reviewer Sheets");
...
function setValuesAtStart(sheetIdx, department, uniqueID, emailId) {
  ...
  const masterDB = SpreadsheetApp.openById(MASTER_DB_SPREADSHEET_ID);
...
function archiveReviewer(emailId, department) {
  const spreadsheet = SpreadsheetApp.openById(MASTER_DB_SPREADSHEET_ID); //1eJ-7nKeICwYXEBh4oEzLwC6SpkYzCw5ntYxarbD2jxA
