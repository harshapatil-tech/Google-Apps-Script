class UpdateNamesQC {
  ...
  getLatestFileName(subjectKey) {
    ...
    // QC Folder
    const insideSubjectFolder = subjectFolder.getFolders();
    let qcFolder = null;
    while(insideSubjectFolder.hasNext()) {
      const f = insideSubjectFolder.next();
      if (f.getName().trim().toLowerCase() === "final qc") {
        qcFolder = f;
        break
      }
    }
    ...
    const yearFolders       = qcFolder.getFolders();
